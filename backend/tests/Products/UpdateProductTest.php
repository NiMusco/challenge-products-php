...
it('validates payload on PUT /productos/{id}', function () {
    $created = $this->api('POST', '/productos', $this->productPayload());
    expect($created['status'])->toBe(201);

    $response = $this->api('PUT', '/productos/' . $created['body']['id'], [
        'nombre' => '',
        'descripcion' => '',
        'precio' => -1,
    ]);
...
it('rejects invalid JSON on PUT /productos/{id}', function () {
    $created = $this->api('POST', '/productos', $this->productPayload());
    expect($created['status'])->toBe(201);

    $response = $this->api('PUT', '/productos/' . $created['body']['id'], '{broken');
